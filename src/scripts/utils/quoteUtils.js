...
export function buildQuoteText() {
  ...
  const taxable  = subtotal + getMembershipCost() + m365Cost;
  const tax      = taxable * store.tax;

  const m365Line = m365Cost
    ? [`${getM365Label()}: $${m365Cost.toFixed(2)}/yr`]
    : [];
